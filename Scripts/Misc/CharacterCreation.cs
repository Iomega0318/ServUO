		...
		private static void AddBackpack(Mobile m)
		{
            ...
            if ( ac != null )
            {
	            ...
	            //Noob one time only pack loot
					...
					PlaceItemIn( pack, 44, 125, new PersonalBlessDeedToken ());
					PlaceItemIn( pack, 44, 90, new BankCheck (60000));
                //Region Ethy (Method used to hue item on creation)
					EtherealHorse mount = new EtherealHorse();
					mount.Hue = ( Utility.RandomMetalHue() );
					PlaceItemIn( pack, 140, 65, (mount));
					//PackItem( mount );
				//End Region
			...
			PlaceItemIn( pack, 44, 65, new daat99.MasterStorage ());
			PlaceItemIn( pack, 60, 65, new Trash4TokensBackpack ());
			PlaceItemIn( pack, 70, 128, new HousePlacementTool ());
			PlaceItemIn( pack, 140, 125, new FullMagerySpellbook ());

			//PackItem(new RedBook("a book", m.Name, 20, true));
			/*PackItem(new Gold(60000)); // Starting gold can be customized here
			//PackItem(new Candle());
			PackItem(new daat99.MasterStorage());
			PackItem(new Trash4TokensBackpack());
			PackItem(new FullMagerySpellbook());
			PackItem(new PersonalBlessDeedToken());
			...
			//Noob one time only bank loot caller
					NewbBank( m );
			//Noob Broadcast
					World.Broadcast( 33, true, ""+m.Name+" has joined for the first time!");
