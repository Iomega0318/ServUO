...
{
	public class ZeldaBoomerang : Item
	{
		[Constructable]
		public ZeldaBoomerang() : base( 0x10E5 )
		{
			Weight = 2.0;
			Hue =  46;
			Name = "a Zelda Boomerang";
		}

		public ZeldaBoomerang( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				if ( from.BeginAction( typeof( ZeldaBoomerang ) ) ) 
				{
		...
		{
			private ZeldaBoomerang m_ZeldaBoomerang;

			public InternalTarget( ZeldaBoomerang boomerang ) : base( 10, false, TargetFlags.Harmful )
			{
				m_ZeldaBoomerang = boomerang;
			}
				...
				if ( targeted is Mobile )
				{
					...
					if ( m != from && from.HarmfulCheck( m ) )
					{
						...
						else
						{
							...
							from.MovingEffect( m_ZeldaBoomerang, 0x10E5, 7, 1, false, false, 0x481, 0 );
		...
		{
 		((Mobile)state).EndAction( typeof( ZeldaBoomerang ) );
		}
	}
}