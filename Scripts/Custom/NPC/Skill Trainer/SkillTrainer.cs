...
{//Begin Mobile Section
	[CorpseName( "skill trainers corpse" )]
	public class SkillTrainer : Mobile
	{
		...
		/* public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			if (dropped is Gold && dropped.Amount == 10000)//How much gold does this cost the player?
			{
			...
			{
					Say(String.Format("I will train thee {0}, for 10,000 gold coins!! The price is non negotiable!", from.Name));//If the gold amount dropped is not at least 10000 the Skill Trainer responds with this phrase.
		...
		{
			if ( !e.Handled && e.Mobile.InRange( this.Location, 12 ) )//How many tiles away can this mobile hear you talk?
			{
				if (e.Speech == "train")//Keyword
				{
					...
					//Say(String.Format("Ok, for 10,000 gold I will teach thee whatever thou wishest to know."));//What response does the skilltrainer have when they hear the keyword? (Use this line instead of above line when charge money is enabled)
				}
         ...
         { 
            ...
            { 
               ...
               { 
                  ...
                  if ( value < 0.0 || value > 100 ) //Max raise for 1 skill
                  { 
                     /* state.Mobile.SendLocalizedMessage( 502452 ); // Value too high. 0-100 only */
					 state.Mobile.SendMessage( 0x26, "Value too high. 0-100 only" );//0x26 red hue
                  } 
                  else if ( ( count + value ) >  700 ) //Skillcap allowed
                  { 
                  state.Mobile.SendMessage( 0x26, "You can not exceed the skill cap.  Try setting another skill lower first." ); //0x26 red hue
                  } 
                  else 
                  { 
                     m_Skill.Base = value; 
               ...
               { 
                  state.Mobile.SendMessage( 0x26, "Bad format. ###.# expected" ); //0x26 red hue
               } 
            } 
         } 
