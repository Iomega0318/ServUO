...
namespace Server.Achievements
{
    ...
    public class AchievementSystem
    {
        ...
        public static IEnumerable<Achievement> GetAchievements(AchievementProfile profile, AchievementType type, bool showLocked)
        {
            ...
            {
                if ((type == AchievementType.All || (a.Type & type) != 0) && (profile == null || !a.Locked || showLocked || profile.HasUnlocked(a)))
                {
                    yield return a;
                }
            }
        ...
        public static void OnSave(WorldSaveEventArgs e)
        {
            Persistence.Serialize(
                ...
                writer =>
                {
                    ...
                    TimedAchievementTracker.Save(writer);
                });
        }

        public static void OnLoad()
        {
            Utility.WriteConsoleColor(Enabled ? ConsoleColor.Green : ConsoleColor.Red, "Achievement System Saved Data Loading...");
            ...
            Persistence.Deserialize(
                ...
                reader =>
                {
                    ...
                    if (version > 0)
                    {
                        TimedAchievementTracker.Load(reader);
                    }
                });
        ...
        public static void PruneProfiles()
        {
            ...
            foreach (var str in Profiles.Keys)
            {
                if (Accounts.GetAccount(str) == null)
                {
                    toRemove.Add(str);
                }
            }
        ...
        public static void LoadAchievements()
        {
            if (Enabled)
            {
                Utility.WriteConsoleColor(ConsoleColor.Green, String.Format("Loading Achievements..."));
            }

            var time = Core.TickCount;
            ...
            if (Enabled)
            {
                time = Core.TickCount - time;

                Utility.WriteConsoleColor(ConsoleColor.Green, String.Format("{0} Achievements loaded: {1} millisconds.", Achievements.Count, time));
            }
        }
        ...
        private static void LoadQuestAchievements()
        {
            ...
            foreach (Assembly assembly in ScriptCompiler.Assemblies)
            {
                ...
                {
                    if ((!type.IsSubclassOf(typeof(BaseQuest)) && !type.IsSubclassOf(typeof(QuestSystem))) || type.Name == "QuestSystemStub")
                    {
                        continue;
                    }
