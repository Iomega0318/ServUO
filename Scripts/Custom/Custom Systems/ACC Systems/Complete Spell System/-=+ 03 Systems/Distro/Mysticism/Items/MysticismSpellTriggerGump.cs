...
{
	public class MysticismSpellTriggerGump : Gump
	{
		public MysticismSpellTriggerGump( Mobile m ) : base( 0, 0 )
		{
			Closable = true;
			Disposable = true;
			Dragable = true;
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( !(info.ButtonID >= 677 && info.ButtonID <= 692) )
				from.SendMessage( "There was an error in your spell choice, please try again." );
			else if ( from.Backpack != null )
			{
				Item[] stones = from.Backpack.FindItemsByType( typeof( MysticismSpellStone ) );
