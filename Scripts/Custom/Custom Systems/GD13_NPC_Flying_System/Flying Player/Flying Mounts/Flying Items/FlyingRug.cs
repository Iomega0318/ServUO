...
{
    public class MagicFlyingRug : Item
    {
        [Constructable]
        public MagicFlyingRug() : base(0xA57)
        {
        ...
        public MagicFlyingRug(Serial serial) : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            Random RugRnd = new Random();
            int rndRug = RugRnd.Next(7);
            ...
            if (!pm.Flying && this.IsChildOf(pm.Backpack))
            {
                if (pm.CantWalk == false)
                {
                    ...
                    FlyingTimer f_timer = new FlyingTimer(pm, rndRug);

                    f_timer.Start();

                    this.Visible = false;
                    this.Name = "Magic Flying Rug-Active";
                }
            }
            else
            {
