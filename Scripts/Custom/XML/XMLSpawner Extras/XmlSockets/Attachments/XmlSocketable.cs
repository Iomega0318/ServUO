    ...
    public class XmlSocketable : XmlAttachment
    {
		...
		public override string OnIdentify(Mobile from)
		{
            ...
            if(MaxSockets == nSockets)
            {
                ...
                return "<basefont color=#38761d>Cannot be socketed any further</basefont>";
            }

            if(MaxSockets > 0)
            {
                msg = String.Format("<basefont color=#38761d>Maximum sockets allowed is {0}</basefont>", MaxSockets);
            } else
            if(MaxSockets == 0)
            {
                return "<basefont color=#38761d>You cannot add sockets to this</basefont>";
            } else
            if(MaxSockets == -1)
            {
                msg = String.Format("<basefont color=#38761d>Can be socketed</basefont>");
            }
