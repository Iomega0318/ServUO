...
namespace Xanthos.Utilities
{
	public class NewsGump : Gump
	{
		...
		private const string kGreeting		= "Greetings And Welcome";
		...
		internal class AccountGump : Gump
		{
			public AccountGump( Mobile from, NetState state, bool showOnlyCredits ) : base( 30, 20 )
			{
				...
				if ( !showOnlyCredits )
				{
					...
					AddLabel( 205, 180, NewsGump.kDataColor, "Time of Legends" );// (( state.Flags & 0x10 ) != 0 ) ? "Samurai Empire" : (( state.Flags & 0x08 ) != 0) ? "Age of Shadows" : (( state.Flags & 0x04 ) != 0) ? "Blackthorn's Revenge" : (( state.Flags & 0x02 ) != 0 ) ? "Third Dawn" : (( state.Flags & 0x01 ) != 0 ) ? "Renaissance" : "The Second Age" );
