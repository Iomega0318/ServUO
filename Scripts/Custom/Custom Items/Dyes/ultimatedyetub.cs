		...
		protected override void OnTarget( Mobile from, object target )
		{
			...
			else if (target is BaseCreature)
			{
				if (m_Amount >= 25)
				{
					...
					{
						c.Hue = m_Item.Hue;
						from.Backpack.ConsumeTotal( typeof( Gold ), 25 ); // Delete the gold
						from.SendMessage( "Removed 25k from backpack and hued your pet." );
					}
					else
				...
				else if (b_Amount >= 25000)
				{
					...
					{
						c.Hue = m_Item.Hue;
						from.Backpack.ConsumeTotal( typeof( Gold ), 75 ); // Delete the gold
						from.SendMessage( "Removed 75 gold from your bank and hued your pet." );
					}
					else
						from.SendMessage("You can only dye animals whom you control!");
				}
		...
		{
			{
				...
				from.SendMessage( "& You may also hue your pets for 75 gp each." );
			}
		}
