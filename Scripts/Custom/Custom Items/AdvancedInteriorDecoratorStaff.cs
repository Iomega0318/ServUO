...
{
    public enum AdminDecorateCommand
    {
        None,
        Secure,
        Lockdown,
    ...
    public class AdminInteriorDecorator : Item
    {
        //        public override int LabelNumber { get { return 1041280; } } // an interior decorator

        private AdminDecorateCommand m_Command;

        [Constructable]
        public AdminInteriorDecorator()
            : base(0xFC1)
        {
            Name = " An Advance Staff Interior Decorator";
            Weight = 1.0;
            LootType = LootType.Regular;
        }

        public AdminInteriorDecorator(Serial serial)
            : base(serial)
        {
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public AdminDecorateCommand Command
        {
        ...
        public static bool CheckUse(AdminInteriorDecorator tool, Mobile from)
        {
            if (!InHouse(from))
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (!InHouse(from))
                m_Command = AdminDecorateCommand.GetHue;

            if (from.FindGump(typeof(InternalGump)) == null)
                from.SendGump(new InternalGump(from, this));

            if (m_Command != AdminDecorateCommand.None)
                from.Target = new InternalTarget(this);
        }

        private class InternalGump : Gump
        {
            private readonly AdminInteriorDecorator m_Decorator;

            public InternalGump(Mobile from, AdminInteriorDecorator decorator)
                : base(150, 50)
            {
                ...
                AddButton(40, 45, (decorator.Command == AdminDecorateCommand.Secure ? 2154 : 2152), 2154, 1, GumpButtonType.Reply, 0);
                AddLabel(80, 50, 0, @"Secure"); //secure

                AddButton(40, 85, (decorator.Command == AdminDecorateCommand.Lockdown ? 2154 : 2152), 2154, 2, GumpButtonType.Reply, 0);
                AddLabel(80, 90, 0, @"Lockdown"); // lockdown

                AddButton(40, 125, (decorator.Command == AdminDecorateCommand.Release ? 2154 : 2152), 2154, 3, GumpButtonType.Reply, 0);
                AddLabel(80, 130, 0, @"Release"); // release

                AddButton(40, 165, (decorator.Command == AdminDecorateCommand.Turn ? 2154 : 2152), 2154, 4, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 170, 70, 40, 1018323, false, false); // Turn

                AddButton(40, 205, (decorator.Command == AdminDecorateCommand.Up ? 2154 : 2152), 2154, 5, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 210, 70, 40, 1018324, false, false); // Up

                AddButton(40, 245, (decorator.Command == AdminDecorateCommand.Down ? 2154 : 2152), 2154, 6, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 250, 70, 40, 1018325, false, false); // Down

                AddButton(40, 285, (decorator.Command == AdminDecorateCommand.North ? 2154 : 2152), 2154, 7, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 290, 70, 40, 1075389, false, false); // north

                AddButton(40, 325, (decorator.Command == AdminDecorateCommand.East ? 2154 : 2152), 2154, 8, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 330, 70, 40, 1075387, false, false); // east

                AddButton(40, 365, (decorator.Command == AdminDecorateCommand.South ? 2154 : 2152), 2154, 9, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 370, 70, 40, 1075386, false, false); // south

                AddButton(40, 405, (decorator.Command == AdminDecorateCommand.West ? 2154 : 2152), 2154, 10, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 410, 70, 40, 1075390, false, false); // west

                AddButton(40, 445, (decorator.Command == AdminDecorateCommand.GetHue ? 2154 : 2152), 2154, 11, GumpButtonType.Reply, 0);
                AddHtmlLocalized(80, 450, 100, 20, 1158863, false, false); // Get Hue

                AddButton(40, 485, (decorator.Command == AdminDecorateCommand.Close ? 2154 : 2152), 2154, 13, GumpButtonType.Reply, 0);
            ...
            {
                AdminDecorateCommand command = AdminDecorateCommand.None;
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(AdminInteriorDecorator decorator)
            ...
            {
                if (m_Decorator.Command == AdminDecorateCommand.GetHue)
                {
                    int hue = 0;
                ...
                else if (targeted is Item /*&& CheckUse(m_Decorator, from)*/)
                {
                    ...
                    if (m_Decorator.Command == AdminDecorateCommand.Turn && IsKingsCollection(item))
                    {
                    ...
                    else if (item is AddonComponent || item is AddonContainerComponent || item is BaseAddonContainer)
                    {
                        ...
                        if (m_Decorator.Command == AdminDecorateCommand.Turn)
                        {
                    ...
                    else if (item is Banner && m_Decorator.Command != AdminDecorateCommand.Turn)
                    {
                        isDecorableComponent = true;
                    }
                    ...
                    {
                        if (item is AddonComponent && m_Decorator.Command == AdminDecorateCommand.Turn)
                            from.SendLocalizedMessage(1042273); // You cannot turn that.
                        else if (item is AddonComponent && m_Decorator.Command == AdminDecorateCommand.Up)
                            from.SendLocalizedMessage(1042274); // You cannot raise it up any higher.
                        else if (item is AddonComponent && m_Decorator.Command == AdminDecorateCommand.Down)
                            from.SendLocalizedMessage(1042275); // You cannot lower it down any further.
                        else if (m_Decorator.Command == AdminDecorateCommand.Secure)
                            Secure(item, from);
                        else if (m_Decorator.Command == AdminDecorateCommand.Lockdown)
                            Lockdown(item, from);
                        else
                    ...
                    else
                    {
                        ...
                        {
                            case AdminDecorateCommand.None:
                                None(item, from);
                                break;
                            case AdminDecorateCommand.Secure:
                                Secure(item, from);
                                break;
                            case AdminDecorateCommand.Lockdown:
                                Lockdown(item, from);
                                break;
                            case AdminDecorateCommand.Release:
                                Release(item, from);
                                break;
                            case AdminDecorateCommand.Turn:
                                Turn(item, from);
                                break;
                            case AdminDecorateCommand.Up:
                                Up(item, from);
                                break;
                            case AdminDecorateCommand.Down:
                                Down(item, from);
                                break;
                            case AdminDecorateCommand.North:
                                North(item, from);
                                break;
                            case AdminDecorateCommand.East:
                                East(item, from);
                                break;
                            case AdminDecorateCommand.South:
                                South(item, from);
                                break;
                            case AdminDecorateCommand.West:
                                West(item, from);
                                break;
                            case AdminDecorateCommand.GetHue:
                                GetHue(item, from);
                                break;
                            case AdminDecorateCommand.Close:
                                Close(item, from);
                                break;
                        }
                    }
            ...
            protected override void OnTargetCancel(Mobile from, TargetCancelType cancelType)
            {
                if (cancelType == TargetCancelType.Canceled)
                    from.CloseGump(typeof(AdminInteriorDecorator.InternalGump));
            }
