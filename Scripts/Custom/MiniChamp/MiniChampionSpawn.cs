...
{   
    public class MiniChamp : Item
    {
        ...
        public static void GenStoneRuins_OnCommand(CommandEventArgs e)
        {
            ...
            e.Mobile.SendMessage("Mini Champion generation complete.");
        }
        ...
        public int Kills
        {
            ...
            set
            {
				if (this.m_Type == MiniChampType.MeraktusTheTormented)
				{
                	...
                	else if(this.m_Level == 2 && value == 1 )
					{
						Map map = Map.Malas; 
        ...
        public Mobile Champion2
        {
            get
            {
                return this.m_Champion2;
            }
            set
            {
                this.m_Champion2 = value;
            }
        }
        ...
        public static Item GiveEssense(MiniChampType mctype)
        {
            Item ess = new EssencePrecision();

            if (mctype == MiniChampType.CrimsonVeins)
                ess = new EssencePrecision();
            else if (mctype == MiniChampType.FairyDragonLair)
                ess = new EssenceDiligence();
            else if (mctype == MiniChampType.AbyssalLair)
                ess = new EssenceAchievement();
            else if (mctype == MiniChampType.DiscardedCavernClanRibbon)
                ess = new EssenceBalance();
            else if (mctype == MiniChampType.DiscardedCavernClanScratch)
                ess = new EssenceBalance();
            else if (mctype == MiniChampType.DiscardedCavernClanChitter)
                ess = new EssenceBalance();
            else if (mctype == MiniChampType.PassageofTears)
                ess = new EssenceSingularity();
            else if (mctype == MiniChampType.LandsoftheLich)
                ess = new EssenceDirection();
            else if (mctype == MiniChampType.SecretGarden)
                ess = new EssenceFeeling();
            else if (mctype == MiniChampType.FireTemple)
                ess = new EssenceOrder();
            else if (mctype == MiniChampType.EnslavedGoblins)
                ess = new EssenceControl();
            else if (mctype == MiniChampType.SkeletalDragon)
                ess = new EssencePersistence();
            else if (mctype == MiniChampType.LavaCaldera)
                ess = new EssencePassion();
			else if (mctype == MiniChampType.MeraktusTheTormented)
                ess = new EssencePassion();
        ...
        public void Respawn()
        {
            ...
            while (Spwnd < spawnn)
            {
                ...
                // Supply Spawns with a chance to carry Essense
                /*if (Utility.RandomDouble() < 0.07)
                {
                    BaseCreature tmpbc = m as BaseCreature;
                    tmpbc.PackItem(MiniChamp.GiveEssense(this.Type));
                }*/
